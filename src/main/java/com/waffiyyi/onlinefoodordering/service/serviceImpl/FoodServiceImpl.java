...
public class FoodServiceImpl implements FoodService {
  ...
  public Food createFood(CreateFoodRequest req, Category category,
    ...
    if (ingredientItems == null || ingredientItems.isEmpty()) {
      throw new BadRequestException("Please add the food's ingredient items",
                                    HttpStatus.BAD_REQUEST);
    }

    for (IngredientsItem ingredientItem : ingredientItems) {
      if (ingredientItem.getCategory() == null || !ingredientCategoryRepository.existsById(
         ingredientItem.getCategory().getId())) {
        throw new BadRequestException(
           "Ingredient category with id" + ingredientItem.getCategory().getId() + " does not exist",
           HttpStatus.BAD_REQUEST);
      }
      if (ingredientItem.getId() == null || !ingredientItemRepository.existsById(
         ingredientItem.getId())) {
        throw new BadRequestException(
           "Ingredient item with ID " + ingredientItem.getId() + " does not exist",
           HttpStatus.BAD_REQUEST);
      }
    }
  ...
  private List<Food> filterByNonVeg(List<Food> foods, boolean isNonVeg) {
    return foods.stream().filter(food -> food.isVegetarian() == false).collect(
       Collectors.toList());
  }
  ...
  public Food findFoodById(Long foodId) {
    return foodRepository.findById(foodId).orElseThrow(
       () -> new ResourceNotFoundException("Food not found", HttpStatus.NOT_FOUND));
  }
